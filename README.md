...
## 2.14.0-alpha Player Crafting, Durability, Repair & Tabbed Item Management UI

v2.14 is a major player-item systems update. Every `AARPGCharacter` now inherits a replicated **Crafting** component that reuses the framework's existing `ARPGRecipeDefinition` Data Assets for secure personal crafting. Recipes are explicitly exposed through `Crafting -> Player Recipes`; recipes with a `Required Station Tag` remain station-only. Craft requests are server-authoritative, validate skills/ingredients/output capacity, consume only one craft's ingredients at a time, use synchronized timer-based progress, refund the currently committed ingredients on cancellation/failure, award existing Skill XP/EventRouter crafting progress, and support safe batches without a permanent Tick.

Recipes can now select the actual **Item Definition asset** directly for each ingredient/output while retaining the legacy `ItemId` field for old content. Existing station crafting resolves both forms too, so projects can migrate gradually. A Stone Axe recipe can therefore be authored as Wood x2 + Stone x3 -> Stone Axe x1 entirely from Data Assets; a Health Potion recipe can use Herb + Water/Vial -> Health Potion the same way.

Equipment durability is now functional per runtime Inventory `InstanceId`. Durable items are unique stacks, expose current/max/percent/broken state, cannot be equipped while broken, and can auto-unequip when they break. Wear is context-authorized: a sword with **Lose Durability On Combat Hit** loses its configured amount only after combat actually applies positive damage; an axe with **Lose Durability On Gathering Hit** loses wear only after a real successful tree chop. Misses, blocked/zero-damage results and failed gathering attempts do not consume durability. The generic Inventory durability Blueprint API is deliberately reusable by future mining/pickaxe, fishing, harvesting and project-specific systems.

Repair is integrated into the same Crafting subsystem. Durable Item Definitions expose full-repair material costs, optional proportional cost scaling by missing durability, free-repair opt-in and broken-item behavior. The server validates owned exact item instance + available unequipped repair materials, consumes the calculated materials, and restores that instance to authored Max Durability.

The ready native UI is now one extensible **Item Management** shell with top-level **Inventory** and **Crafting & Repair** tabs. The Crafting page includes recipe availability, ingredient/output/requirement summaries, quantity +/-/MAX controls, batch Craft, Cancel, synchronized progress, and an internal Repair view listing real damaged equipment and material costs. `InventoryUI` exposes `Crafting Widget Class`, `Crafting Recipe Row Widget Class`, and `Repair Item Row Widget Class` so the entire system can be reskinned with Widget Blueprint subclasses while retaining native logic. The shell is intentionally structured around a switcher/host so later framework systems can add more top-level pages without replacing Inventory again. See `Docs/CRAFTING_DURABILITY_REPAIR.md`.
