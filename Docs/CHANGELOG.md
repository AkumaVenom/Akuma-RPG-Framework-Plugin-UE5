## v2.18.5-alpha — Account-Scoped Single-Player & Listen-Host World Persistence Root Fix

- Fixed world-state leakage between frontend accounts. Character slots were already account-scoped, but `MakeWorldSlotName` still returned one `ARPG_World_<WorldId>` slot for every local account.
- Logged-in standalone sessions now save/load `ARPG_<AccountId>_World_<WorldId>`. Listen servers use the **host AccountId** as the stable authoritative world namespace; remote clients do not own or load local world saves. Guest/no-login and dedicated-server execution retains the legacy unscoped namespace.
- `ARPGGameMode` captures `ActiveWorldSaveWorldId`, `ActiveWorldSaveAccountId`, and `ActiveWorldSaveSlotName` once at BeginPlay so logout/travel or remote connection churn cannot redirect the final world save to another namespace.
- World save advances from **v9 to v10** and stores `ScopeAccountId`. v10 loading rejects a payload whose embedded scope does not match the requested account, including explicit SlotOverride callers. All v5-v9 building/storage/production/settlement/path migration branches remain intact. Character save stays **v5**; account-profile metadata stays **v1**.
- Existing shared v9 world slots are **not auto-imported** into a logged-in account because they may contain mixed-account structures and global state. They remain untouched for legacy/Guest compatibility.
- Added `Tools/test_account_scoped_world_persistence_model.py` plus validator coverage for slot isolation, host ownership, client exclusion, schema binding and stable GameMode world context.

## v2.18.4-alpha — Explicit Gameplay GameMode Travel Contract Root Fix

- Replaced the v2.18.3 runtime WorldSettings recovery as the primary fix after project PIE proved that the runtime `DefaultGameMode` can already reflect the stale frontend override.
- Added **Default Gameplay GameMode** under Project Settings -> Game -> Akuma's RPG Framework -> Frontend. It is a soft class reference restricted to `ARPGGameMode` descendants and should point at the project's authored gameplay GameMode Blueprint (for example `ARPG_GameMode`).
- Single Player and Host & Play now validate/load that class before travel and explicitly send Unreal's highest-priority `game=` URL option. This deterministically overrides stale frontend/project/map resolution in UE5.8 PIE.
- Host & Play preserves `listen`, `Port` and `ARPG_LAN` in the same explicit URL contract. Direct-IP clients still do not choose the server's GameMode; the host remains authoritative.
- Missing/invalid gameplay GameMode configuration now fails closed in the Main Menu with a concrete error instead of travelling under an ambiguous GameMode.
- Added travel logging with destination map, resolved gameplay GameMode and final framework-owned options for diagnosis.
- v2.18.3 guarded recovery remains as a secondary fail-safe; v2.18.2 input/identity/bootstrap and v2.17.3 persistence contracts remain unchanged.

## v2.18.3-alpha — Destination-Authored GameMode Travel Recovery Root Fix

- Fixed the project-observed UE5.8 PIE transition where `StartingIslandMap` loaded but `ARPGFrontendGameMode` remained the instantiated runtime GameMode even though the destination map World Settings correctly authored `ARPG_GameMode`.
- `AARPGFrontendGameMode` now validates the loaded world’s `AWorldSettings::DefaultGameMode` during BeginPlay. A genuine frontend map is left untouched; if a non-frontend destination is incorrectly running under the frontend GameMode, the framework performs one absolute reopen of that same map with the destination-authored class explicitly supplied as Unreal’s internal `game=` travel option.
- Added `ARPG_GameModeRecovery=1` loop protection and detailed `LogARPG` diagnostics. If an external PIE/project URL override still defeats the authored class after recovery, the framework reports the current map/current class/authored class instead of recursively travelling forever.
- Preserved v2.18.2 frontend widget cleanup, GameOnly input restoration, stable pre-travel CharacterId, non-reentrant local spawn/possession and fresh-vs-existing persistence behavior. Character save remains v5, world save remains v9 and account-profile metadata remains schema v1.
- Added `Tools/test_frontend_gamemode_travel_recovery_model.py` and validator coverage for authored destination GameMode inspection, frontend-map exclusion, guarded one-shot recovery and absolute travel.

## v2.18.2-alpha — Deterministic Frontend-to-Gameplay Possession & Fresh-Profile Bootstrap Root Fix

- Fixed the real UE5.8 Single Player handoff where the frontend's `FInputModeUIOnly` state could survive travel long enough to leave the gameplay pawn locally controlled but unable to move/look. All Single Player / Host & Play / Join paths now call `PrepareForGameplayTravel`, remove the frontend widget, restore `FInputModeGameOnly`, hide the mouse cursor and clear move/look input suppression before travel.
- Added a gameplay-side safety net in `AARPGPlayerController`: local gameplay controllers restore GameOnly input on BeginPlay whenever entering an ARPG gameplay authority world or an active frontend travel session.
- Accounts now establish a stable CharacterId during account creation. Existing/migrated accounts repair a missing LastCharacterId on login through `GetOrCreateLastCharacterId`, and every frontend travel validates that stable identity before leaving the menu.
- Hardened persistence fallback so a logged-in local player with no controller-approved identity still resolves/repairs the account's stable CharacterId instead of allowing character identity to depend on pawn BeginPlay timing.
- Removed local profile-acceptance spawn re-entry. `AcceptProfileIdentityOnAuthority` only resumes spawning when `BeginProfileIdentityHandshake` explicitly marked a remote connection as waiting; standalone/listen-host local spawn remains synchronously owned by `ARPGGameMode::HandleStartingNewPlayer_Implementation`.
- Preserved the v2.17.3 persistence contract: fresh/no-save characters seed Starting Items + Quick Access once, existing saves (including intentionally empty Inventory) restore exactly, and load failures remain overwrite-protected. Character save stays v5; world save stays v9; account-profile schema stays v1.
- Added `Tools/test_frontend_gameplay_handoff_model.py` and validator coverage for frontend input relinquish, gameplay input restoration, pre-travel stable CharacterId, non-reentrant local spawn and fresh-profile bootstrap.

## v2.18.1-alpha — UE5.8 Frontend/Network Compile Compatibility Root Fix

- Fixed the project-reported UE5.8 C++ build failure in `UARPGNetworkSubsystem`. The implementation now uses Unreal's actual network/travel failure delegate aliases, `ENetworkFailure::Type` and `ETravelFailure::Type`, matching the already-correct public declarations and `FEngine` delegate signatures. This removes the parser cascade that made `FailureType`, `ErrorString` and `HandleFailureAndMaybeReturn` appear invalid.
- Fixed UE5.8 UHT-generated compile failures for the reskinnable Login/Main Menu Blueprint events. Reflected `FText`/`FString` event inputs are now declared as `const FText&` / `const FString&`, exactly matching the signatures emitted into `ARPGFrontendWidgets.gen.cpp`; the event names and Blueprint-facing behavior are unchanged.
- Extended `Tools/test_packaging_compile_compat_model.py` so these exact UE5.8 enum/delegate and UHT const-reference contracts are regression-locked rather than relying only on source/model validation.
- No frontend flow, local-account security boundary, direct-IP profile handshake, host-authoritative remote persistence, Inventory/Quick Access bootstrap, `SaveNow` routing, replication policy or save schema changed. Character save remains **v5**, world save remains **v9**, account-profile metadata remains **v1**.

## v2.18.0-alpha — Native Login/Main Menu + Direct-IP Profile Identity Networking

- Added a complete native, Blueprint-reskinnable **frontend/login flow** for a blank project Main Menu map. `AARPGFrontendGameMode` intentionally spawns no gameplay pawn; `AARPGFrontendPlayerController` supplies working `UARPGLoginWidget` and `UARPGMainMenuWidget` fallbacks with mouse/UI-only input while exposing Widget Classes and Blueprint presentation events for project skins.
- Added local **Create Account / Login / Logout** frontend actions. Account creation is transactional: the existing salted local verifier/index entry and a new non-secret `UARPGAccountProfileSave` (`ARPG_Account_<AccountId>`, profile schema v1) must both be established or the reserved account entry is rolled back. Existing pre-v2.18 local accounts lazily create profile metadata on successful login.
- Password handling remains local-only. Raw passwords are never stored in character/world/account-profile saves and are **never sent in network RPCs**. The direct-IP identity handshake submits only the locally verified AccountId, display username and selected/last CharacterId. The local password system remains trusted/local profile separation, not production Internet authentication.
- Added ready Main Menu actions for **Single Player**, **Host & Play** listen server, typed **Join by IP/hostname**, Logout and Quit. Listen port/LAN mode and last join address are persisted as non-secret account preferences. Address normalization supports hostnames, IPv4, bracketed IPv6 and default-port completion while rejecting travel-option injection characters.
- Expanded `UARPGNetworkSubsystem` with explicit frontend/session state (`SinglePlayer`, `ListenHost`, `DirectIPClient` and connection states), configurable frontend/gameplay maps, normalized direct-IP travel, disconnect/return-to-menu flow, and Unreal network/travel-failure handling with a retained user-facing failure message.
- Added Project Settings defaults for Main Menu map, Gameplay map, listen port, maximum players, default LAN mode, profile-handshake timeout, login requirement and optional automatic return to Main Menu after network/travel failure.
- Made multiplayer profile identity a **pre-pawn authority boundary**. Gameplay `AARPGGameMode` can hold a connection at `AARPGPlayerController` until profile identity is accepted. The listen server rejects malformed/duplicate active AccountIds or CharacterIds, generates a CharacterId for a first character when needed, binds accepted identity immutably to the connection, and only then enters normal pawn startup/persistence.
- Reworked character-save namespace resolution for listen-server multiplayer. `UARPGSaveSubsystem` now supports account-specific and actor-specific slot resolution; a remote player's server-authoritative save is keyed by that connection's accepted `AccountId + CharacterId`, never by the host process's local Account Subsystem. Host/local fallback is restricted to the locally controlled character.
- Persistence bootstrap now consumes the server-approved PlayerController CharacterId before resolving the character save. The proven v2.17.3 startup states remain authoritative: fresh profile seeds defaults once, an existing save restores exact Inventory/Quick Access including intentionally empty state, and an unreadable existing save suppresses default injection/automatic overwrite.
- Remote direct-IP character snapshots are deliberately **host-authoritative**. Reconnecting the same local profile to the same host restores that host's snapshot. Portable characters across unrelated hosts require a trusted backend/cloud/platform identity service; the framework does not accept a client-supplied Inventory/state blob as authority.
- Existing Blueprint/UI `Persistence -> SaveNow` remains valid for joined clients: the replicated player Persistence component routes a reliable owner request to the listen host, authority writes that connection's account-scoped character snapshot, and `OnManualCharacterSaveResult` returns success/failure to the owning client. Native AI disables this RPC replication path to avoid unnecessary replicated persistence subobjects.
- Existing replicated gameplay systems retain their server-authoritative paths after identity binding: Inventory/equipment/Quick Access, combat, Stats/progression/Mining, building/ownership, Doors/Windows/Lights, Storage, Production, Settlements, Woodcutting/Mining and chat. This update does not claim to automatically replicate arbitrary new project Blueprint variables that were never authored for replication.
- Added `Docs/FRONTEND_LOGIN_NETWORKING.md` with complete blank-level setup, native/custom UI binding names, account flow, direct-IP hosting/joining, profile handshake, save ownership/security boundaries, limitations and multiplayer QA procedure.
- Added `Tools/test_frontend_login_direct_ip_network_model.py`, `Tools/test_multiplayer_replication_contract_model.py` and validator invariants for secret isolation, spawn gating, identity immutability/duplicate rejection, actor-specific save namespaces, frontend configuration and the existing ready-framework replication/RPC contract. **Character save remains v5, world save remains v9; account-profile metadata is schema v1.**

## v2.17.3-alpha — Deterministic Fresh-Character Persistence Bootstrap Root Fix

- Fixed the opposite side of the automatic Inventory persistence failure exposed after enabling `Auto Load on Begin Play`: a genuinely new/no-save character could remain without authored Starting Items because Inventory waited for persistence while the persistence owner path could exit without resolving the startup transaction.
- Replaced the ambiguous boolean startup path with explicit `EARPGInitialCharacterPersistenceState`: `Unresolved`, `Auto Load Disabled`, `Fresh Character - No Save`, `Loaded Existing Save`, and `Existing Save - Load Failed`. Auto-load-enabled authoritative player characters now always leave `Unresolved` instead of silently deadlocking Inventory initialization.
- `AttemptAutoLoad` now checks `DoesCharacterSaveExist` before invoking `LoadCharacter`. A proven **no-save** result is therefore distinct from an existing slot that failed to deserialize. Fresh characters seed authored Starting Items/auto-equip/Quick Access defaults; existing loaded characters restore exact saved state including intentionally empty Inventory; existing-but-unreadable slots suppress both defaults and automatic overwrite.
- Fresh-character initialization immediately establishes the first complete character snapshot when any automatic persistence mode is enabled. The initial defaults no longer depend on the later debounce, periodic autosave or PIE EndPlay to become a real save.
- Removed the class-only persistence BeginPlay early-return that could leave a project-owned playable subclass waiting forever. `AARPGAICharacter` remains excluded while AI-controlled, but a project may possess such a subclass with a player controller and use account-character persistence. The save subsystem uses the same runtime ownership rule.
- Added runtime diagnostics `Initial Character Persistence State`, `Initial Resolved Character Id` and `Initial Resolved Character Save Slot` so project PIE can directly verify fresh/load/failure state and the exact slot identity.
- Retained v2.17.2 state-driven Inventory/Quick Access debounce, synchronous character writes, final EndPlay commit and load-failure overwrite protection. Character save remains **v5** and world save remains **v9**; no migration is required.

## v2.17.2-alpha — Transactional Automatic Inventory + Quick Access Persistence Root Fix

- Fixed the remaining project-side relog/PIE reset where Inventory and Quick Access could still return to the authored Starting Items layout even after v2.17.1. The missing guarantee was on the **write side**: character state was still only committed by the long periodic autosave or EndPlay, so a real Inventory/hotbar mutation was not itself a persistence transaction.
- `UARPGPersistenceComponent` now subscribes on authority to `OnInventoryChanged`, `OnQuickAccessChanged` and `OnActiveQuickAccessSlotChanged`. These events mark the complete character snapshot dirty and restart one configurable debounce timer. The default 1.5-second quiet period coalesces rapid storage transfers, drag/drop, Mining loot, consumable use, equipment changes and hotbar edits into one save.
- Added editor-exposed `Persistence | Automatic State Save -> Save Inventory And Quick Access Changes Automatically` (default **true**) and `Character State Save Debounce Seconds`, plus runtime `Character State Save Pending` and `Automatic Character State Save Count` diagnostics. `Flush Pending Character State Save` is Blueprint-callable for explicit checkpoints/tests.
- Automatic mutation saves are gated behind the initial auto-load decision, so broadcasts generated while restoring `D.Inventory` / saved Quick Access cannot write partial load state back to disk. Fresh-character Starting Items are applied only after initial persistence resolution and therefore become the first normal dirty snapshot.
- Serialized **all character saves synchronously** in the ready persistence path. `SaveNow`, periodic character autosave, debounced mutation saves and EndPlay use the synchronous character-save path; the lower-level `SaveCharacter` implementation is also serialized. This removes stale async completion ordering where an older character snapshot could finish after a newer one and resurrect stale/default Inventory. World saves remain asynchronous.
- `Save On EndPlay` now commits for every player-character EndPlay reason, including `Destroyed`, while retaining the initial-load and load-failure safety guards. This removes dependency on whether PIE, respawn or project teardown reports `EndPlayInEditor`, `Quit`, `LevelTransition` or explicit actor destruction.
- Retained v2.17.1 existing-save authority: intentionally empty saved Inventories stay empty, Starting Items are creation defaults only, and a detected unreadable save suppresses automatic overwrite.
- Character save schema remains **v5** and world save remains **v9**. No migration is required. v2.17.0 Mining and all settlement/building/production systems remain unchanged.

## v2.17.1-alpha — Automatic Player Inventory Persistence Root Fix

- Fixed automatic player Inventory restoration being able to fall back to the Blueprint `Starting Items` loadout after BeginPlay. The previous implementation inferred “new character” from an empty runtime Inventory after a fixed deferred delay; that is not a valid persistence test because an existing saved character may intentionally own zero items, and Guest identity recovery can legitimately require additional deferred startup work.
- Replaced the timing heuristic with explicit persistence ownership. `UARPGPersistenceComponent` now resolves the stable CharacterId, checks whether that character save slot exists, performs the initial load, and then calls `ResolveStartingItemsAfterInitialPersistence`. Only a genuinely fresh/no-save character receives automatic Starting Items. Existing saved characters suppress starter seeding even when the saved Inventory is completely empty.
- Added Blueprint-visible initial persistence runtime state: `Initial Auto Load Resolved`, `Loaded Existing Character Save`, `Initial Character Save Found`, `Automatic Save Suppressed After Load Failure`, plus `On Initial Character Persistence Resolved`. This makes startup state inspectable without exposing mutable save authority.
- Hardened detected-save load failure. If a character save slot exists but cannot be loaded, the framework does **not** inject Starting Items and automatically suppresses periodic/end-play writes for that session, preventing a corrupt/unreadable save from being overwritten by default runtime state. Explicit manual save APIs remain available if a project deliberately chooses to replace it.
- Added `SaveCharacterImmediate` / `SaveNowImmediate` and changed automatic `Save On EndPlay` to synchronously commit character state before world/PIE teardown. Normal timed/manual gameplay saves remain asynchronous to avoid blocking play. Automatic autosave also refuses to write before the initial auto-load decision resolves.
- Kept loaded Inventory order authoritative: `LoadCharacter` still restores `D.Inventory` through `ReplaceInventory` before Quick Access repair and before final current-vital clamping. Equipment instance GUIDs, durability, equipped flags, quantities and exact Item Definition references therefore round-trip exactly as before.
- Character save remains **v5** and world save remains **v9**. No migration is required, and v2.17.0 Mining/rock/pickaxe progression is unchanged.
- Added `Tools/test_inventory_automatic_persistence_model.py`, validator invariants and documentation/PIE acceptance coverage for fresh-character seeding, modified saved Inventory, intentionally empty saved Inventory, synchronous teardown save and load-failure overwrite protection.

## v2.17.0-alpha — First-Class 1–99 Mining & Mineable Resource Nodes

- Added native `UARPGMiningComponent` to the ready `AARPGCharacter`. Mining is fully Blueprint-exposed with level/XP/progress queries, configurable 1–99 progression, exact equipped-runtime pickaxe resolution, tool power/tier/durability, range/trace settings, repeated swing timing, animation/audio hooks, state/events and server RPC authority.
- Added `AARPGMineableRock`, a replicated Blueprintable resource actor intended for hand placement or Unreal **Actor Foliage**. Designers can author arrays of Rock Mesh Variations plus authority-selected random uniform scale and yaw, optional depleted/rubble presentation, Mining Health/Resistance, Required Mining Level, required tool tag/tier, respawn, Niagara/Cascade/audio and Blueprint calls/events without a custom harvesting Blueprint.
- Added the generic `EARPGSkillXPModel` selector. Existing Skill Definitions remain on `Framework Power Curve` by default and any authored `XP Required Per Level` keys remain authoritative. The new `RuneScape-Style 1-99 Curve` reproduces the familiar cumulative 1–99 thresholds (Level 2 = 83 total XP; Level 99 = 13,034,431 total XP) while converting them into the framework's existing per-current-level Skill-state representation. Native Mining uses this model by default without requiring a Skill Definition.
- Added action/progression-style node health. Every authoritative Mining strike applies configurable Mining Power against Rock Mining Resistance, can award XP and **Successful Strike Drops**, and the final strike additionally awards `XP On Depletion` plus independent **Depletion Drops**. This supports repeated Stone/Ore yield and distinct final-node payloads rather than a single one-shot loot event.
- Added `Bonus Chance Drops` for Gems, crystals, geodes, fossils, rare ore or project-specific finds. Each bonus entry exposes strike/depletion/both trigger, quantity range, Base Chance, Required Mining Level, Minimum Tool Tier and optional Mining-level/tool-tier chance scaling with a clamped per-rock maximum. `Get Effective Bonus Drop Chance` is Blueprint Pure for UI/debug inspection.
- Added exact equipped pickaxe validation using the existing Inventory/Equipment runtime-instance contract: valid GUID, positive quantity, equipped flag, matching Equipment Slot, non-broken durability, required Gathering/Item tag and minimum Gathering Tool Tier. Highest valid tier/power wins; durability loss is charged only to the exact equipped instance after a successful strike.
- Integrated Mining into existing **Basic Attack** before ordinary combat Stamina/Mana spending. With a valid pickaxe and Mineable Rock context, one Basic Attack becomes one rate-limited free Mining strike; real non-rock combat targets continue through normal combat. Existing Woodcutting interception remains first and unchanged.
- Added direct interaction mining plus `AARPGCharacter::InteractWorld()`. Projects may bind one Interact action that tries Mineable Rock -> ARPGTree -> built structure, while existing `InteractBuiltStructure`, Woodcutting and Mining-specific wrappers remain available. `Start Mining From View` can repeat automatically until depletion/cancel/range/tool failure.
- Extended v2.16.9 renewable-resource building replacement to framework Mineable Rocks. Foundation placement traces/overlap may pierce `AARPGTree` and `AARPGMineableRock` only; ordinary rocks/props remain blockers. Build occupancy suppresses rock visuals/collision/respawn with no fabricated Mining XP, normal loot, bonus loot or depletion event. Settlement Paths still skip renewable-resource occupancy scans.
- Added standard tags: `Skill.Mining`, `Gathering.MineableRock`, `Item.Tool.Pickaxe`, `Item.Resource.Stone`, `Item.Resource.Ore`, and `Item.Resource.Gem`. Item Definition gathering metadata is shared cleanly by axes and pickaxes.
- Mining Skill progression persists through the existing generic Skills save payload, so character save remains **v5**. Mineable Rock depletion/respawn follows the existing Tree-style runtime renewable-resource model; persistent buildings re-establish suppression and world save remains **v9**. No save migration is required.
- Added `Docs/MINING.md`, editor setup/Actor Foliage/performance/network acceptance documentation, `Tools/test_mining_progression_rock_model.py`, and v2.17.0 validator invariants while retaining all previous Settlement Path, Tree, storage/production, Dynamic Recast and structural regression contracts.

## v2.16.12-alpha — Settlement Path Turn Tangent Stability Root Fix

- Fixed the real PIE corner-deformation failure in the v2.16.11 Settlement Path system. Straight roads were correct, but confirmed turns could fold, spike or invert a spline mesh because the shared endpoint tangent direction was multiplied by the **entire player-authored segment length** and then applied to only the first/last terrain-sampled interval. With the default 125 cm sampling, an 800 cm placement could therefore inject an ~800 cm Hermite tangent into a ~100-125 cm local span.
- `AARPGBuildPathActor` now treats persisted endpoint tangent overrides as **directional intent**. Runtime tangent magnitude is derived from the endpoint's actual adjacent sampled span, preventing overshoot while retaining smooth shared bisector direction at ordinary turns. The public/save field names remain unchanged for Blueprint and serialization compatibility.
- Added a sharp-reversal safety guard. If a requested shared tangent no longer points sufficiently forward through the local endpoint span, the actor falls back to the segment's own travel direction instead of allowing spline fold-back. Normal bends remain smoothed; pathological near-U-turns degrade safely rather than exploding geometry.
- Existing world save schema remains **v9**. v2.16.11 saves that contain oversized tangent magnitudes are automatically normalized during restore, so already-saved Settlement Paths self-heal without a schema migration and are written back with direction-only tangent state on the next save. Character save remains v5.
- The local pooled path preview now tracks the last confirmed segment and applies the same bounded start-turn rule to the pending segment, reducing preview/final discontinuity when authoring bends. No new replicated gameplay state or permanent Tick was added.
- Kept all v2.16.11 contracts intact: continuous first-anchor/next-point authoring, terrain projection/sampling, per-segment cost/ownership/demolition, non-structural placement semantics, Tree suppression exclusion, optional collision, replication and v9 persistence. No Data Asset migration is required; existing `SettlementPath` definitions can be retested unchanged.
- Expanded the Settlement Path regression model and source validator to reject whole-segment tangent magnitude coupling and require bounded adjacent-span tangent handling, restore sanitization and live-preview parity.

## v2.16.11-alpha — Player-Built Settlement Spline Paths

- Added first-class `SettlementPath` as an appended `EARPGBuildPieceKind`, preserving every existing enum value. Path placement is continuous: the first confirmation establishes a server-approved start anchor without spawning/charging a segment, every later confirmation builds one connected segment and advances the anchor, and the session remains active until **Cancel / End Build Mode**.
- Added native `AARPGBuildPathActor`. Each completed path section remains one normal authoritative build actor for Building ID, ownership/faction, health, construction, demolition/refund, replication and persistence, while a native Spline Component generates terrain-conforming `USplineMeshComponent` presentation from the Data Asset's Static Build Mesh. `Actor Class = None` resolves automatically to this native actor.
- Added high-quality Data Asset authoring under `Building | Settlement | Path`: spline-mesh forward axis, cross-section scale, ground offset, terrain sample spacing/trace height/depth, minimum/maximum segment length, tangent scale, optional completed-path collision and shadow casting. The Build Mesh is the deformation source; an optional Preview Mesh can serve as a first-point marker.
- Added a pooled live path preview. Before the first anchor the normal local ghost may show the optional point marker; after confirmation the preview becomes a connected Spline Mesh strip from the last server-confirmed point to the cursor. It uses the same bounded terrain-sampling contract as the finished segment, valid/invalid material parameters, explicit too-short/too-long feedback and reuses Spline Mesh components instead of destroying/recreating them every frame.
- Added smooth multi-segment turns without creating one fragile monolithic spline actor. When authority creates a turn, it derives a shared normalized bisector from the previous/new travel directions and writes matching endpoint tangents to the previous and new independent segment. Tangent overrides replicate and persist so a curved road retains its join quality after late join/load.
- Hardened terrain projection for networks with optional collision: first/next-point projection, live preview sampling and final path sampling all pierce existing `AARPGBuildPathActor` hits to reach the underlying world/support surface. A new road therefore cannot accidentally climb onto an older road merely because project-specific path collision was enabled.
- Added dedicated multiplayer authority and Blueprint state. `ServerBeginSettlementPath`, `ServerPlaceSettlementPathPoint` and `ServerCancelSettlementPath` keep the authoritative previous point server-side; client callbacks advance local state only after success, and a pending-request guard prevents double-submit drift. Every point is reprojected/revalidated for catalogue, range, surface, resources and segment-length limits. Generic one-shot build placement explicitly rejects `SettlementPath`, preventing bypass of the continuous-session contract.
- Settlement Paths are intentionally **non-structural**. They do not participate in structural build-overlap rejection, semantic Foundation/Floor/Wall/Stair occupancy, Tree replacement/respawn suppression or structural Dynamic Recast dirtying. They also skip the per-build Tree notification scan, keeping large decorative road networks from adding unnecessary authority work. Optional path collision remains presentation/project gameplay rather than a structural claim.
- Added native HUD text plus Blueprint hooks: `Cancel Settlement Path Placement`, active/start/pending/last-point queries, `On Settlement Path Session Changed`, `On Settlement Path Segment Placed`, path geometry getters/refresh and `On Path Geometry Changed`. Failure on the first anchor is routed through the same placement-result channel as ordinary building failure for consistent custom UI.
- Advanced world save schema from **v8 to v9**. Each `FARPGPlacedBuildingSave` can persist Settlement Path local start/end endpoints and local endpoint tangent overrides; Spline Mesh sections remain derived presentation and are rebuilt on load. Character save remains v5, older v6 Window/v7 Light/v8 settlement migration behavior remains intact, and all protected v2.15.53 Stair/Wall-family topology plus v2.16.8 Dynamic Recast semantics are unchanged.
- Added `Tools/test_settlement_player_path_spline_model.py`, extended `validate_source.py` with v2.16.11 path invariants/save-version checks, and expanded Settlement/Building/Networking/Feature Matrix/Validation documentation for the complete authoring, authority, performance and PIE acceptance contract.
