...
## Settlement Building, Spline Paths, Storage, Lighting & Autonomous Villagers (v2.16.12)
...
| Runtime-built structural navigation refresh | Implemented | v2.16.8 structural build changes update NavigationSystem octree state and dirty bounded Dynamic Recast regions. Completed Stairs use their real rasterized Recast surface under project Supported Agent settings; automatic framework NavLinkProxy/Simple Nav Link generation is intentionally removed. |
...
| Buildable interactive lighting | Implemented | Additive `Light` Piece Kind. Stick/floor fixtures seat on terrain/Foundations/Floors; wall fixtures attach to either face of completed Wall/WindowWall/Doorway surfaces. Native replicated/persistent On/Off interaction, smooth Point/Spot light + optional emissive fade, Niagara/Cascade modes, optional sounds and no fuel consumption. Fixture collision stays non-blocking so v2.15.53 structural Stair/Wall/Window combinations remain unchanged. |
...
| Authoritative placement | Implemented | Server re-resolves transform/snap and validates catalog membership, snap-target modification access, resources, range, collision, support/slope, faction and territory before spawn. Settlement Paths additionally keep the previous confirmed endpoint on authority, reproject every requested point and reject generic one-shot placement bypasses. |
...
| Building persistence | Implemented | World v9: IDs, transform, health, ownership, construction remaining time, Door/Window/Light state, Settlement Path endpoints/tangent joins, storage and production input/output/queue. v2.15.12 stable Guest ownership migration remains intact. |
...
| Inventory | Implemented | Stacking, add/remove/transfer, exact runtime Item Definition references, replication/server authority, deterministic fresh/no-save Starting Items bootstrap, debounced mutation-driven automatic character saves and serialized relog restoration. |
| Quick Access / hotbar | Implemented | Persistent 1-based active item slots, strict one-slot-per-runtime-instance identity, owned-runtime binding/rebinding, weapon/tool equip switching, food/potion use, cooldowns, cycling, UMG views, owner-only replication, and debounced automatic persistence of slot layout + active slot. |
